...
# Verifies that skills are resolved with correct priority: project > personal > hivemind
...
# 1. Create in hivemind location (lowest priority)
mkdir -p "$HOME/.config/opencode/hivemind/skills/priority-test"
cat > "$HOME/.config/opencode/hivemind/skills/priority-test/SKILL.md" <<'EOF'
---
name: priority-test
description: Hivemind version of priority test skill
---
# Priority Test Skill (Hivemind Version)

This is the HIVEMIND version of the priority test skill.

PRIORITY_MARKER_HIVEMIND_VERSION
EOF
...
if [ -f "$HOME/.config/opencode/hivemind/skills/priority-test/SKILL.md" ]; then
    echo "  [PASS] Hivemind version exists"
else
    echo "  [FAIL] Hivemind version missing"
    exit 1
fi
...
# Test 2: Test that personal overrides hivemind
echo ""
echo "Test 2: Testing personal > hivemind priority..."
...
if echo "$output" | grep -qi "PRIORITY_MARKER_PERSONAL_VERSION"; then
    echo "  [PASS] Personal version loaded (overrides hivemind)"
elif echo "$output" | grep -qi "PRIORITY_MARKER_HIVEMIND_VERSION"; then
    echo "  [FAIL] Hivemind version loaded instead of personal"
    exit 1
else
    echo "  [WARN] Could not verify priority marker in output"
    echo "  Output snippet:"
    echo "$output" | grep -i "priority\|personal\|hivemind" | head -10
fi

# Test 3: Test that project overrides both personal and hivemind
echo ""
echo "Test 3: Testing project > personal > hivemind priority..."
...
if echo "$output" | grep -qi "PRIORITY_MARKER_PROJECT_VERSION"; then
    ...
    echo "  [FAIL] Hivemind version loaded instead of project"
    exit 1
else
    echo "  [WARN] Could not verify priority marker in output"
    echo "  Output snippet:"
...
# Test 4: Test explicit hivemind: prefix bypasses priority
echo ""
echo "Test 4: Testing hivemind: prefix forces hivemind version..."

cd "$TEST_HOME/test-project"
output=$(timeout 60s opencode run --print-logs "Use the use_skill tool to load hivemind:priority-test specifically. Show me the exact content including any PRIORITY_MARKER text." 2>&1) || {
    exit_code=$?
    if [ $exit_code -eq 124 ]; then
        echo "  [FAIL] OpenCode timed out after 60s"
        exit 1
...
if echo "$output" | grep -qi "PRIORITY_MARKER_HIVEMIND_VERSION"; then
    echo "  [PASS] hivemind: prefix correctly forces hivemind version"
elif echo "$output" | grep -qi "PRIORITY_MARKER_PROJECT_VERSION\|PRIORITY_MARKER_PERSONAL_VERSION"; then
    echo "  [FAIL] hivemind: prefix did not force hivemind version"
    exit 1
else
    echo "  [WARN] Could not verify priority marker in output"
fi
